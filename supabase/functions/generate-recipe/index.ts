  ...
  "steps": [
    {
      ...
      "instruction": "<1-3 sentences. EVERY first-mention of any ingredient in this step MUST carry its amount inline — no bare nouns for measured ingredients. The amount string MUST be byte-identical to the ingredients[] row's `amount` field for that ingredient (the UI does a literal find-and-replace to convert units when the cook picks a different one — 'Pour 1 cup of red wine' ⇄ 'Pour 240 ml of red wine'; if the prose says '6 oz' but the ingredients row says '12 tbsp', the rewrite can't fire and the cook reads stale numbers). Brand goes inline too when present.\n\n      WRITE:\n        ✓ 'Pour 1 cup of red wine into the pan and reduce by half'\n        ✓ 'Heat 2 tbsp of unsalted butter in a 10-inch skillet'\n        ✓ 'Add 2 chicken breasts and 4 cloves of garlic'\n        ✓ 'Whisk in 1 cup of buttermilk until smooth'\n      DO NOT WRITE:\n        ✗ 'Pour the wine and reduce'           (bare noun — measurement missing)\n        ✗ 'Add the chicken and garlic'         (bare nouns — both miss amounts)\n        ✗ 'Stir in the buttermilk'             (bare noun — even on the second prep step)\n\n      WITHIN-REASON CARVE-OUTS — these are the ONLY cases where a bare reference is acceptable:\n        • Re-mention of the same ingredient LATER in the same step ('… add the wine, reduce, then deglaze with the wine again' — second mention can be 'the wine'). FIRST mention always carries the amount.\n        • Pronouns and group references ('it', 'them', 'the mixture', 'the dough', 'the sauce', 'everything') after the ingredient was already fully introduced earlier in this step or the immediately preceding step.\n        • Pure-staple shorthand: 'season with salt and pepper' / 'finish with a drizzle of olive oil' is acceptable ONLY when the ingredient is salt, pepper, pepper flakes, or a finishing oil — every other ingredient (including butter, wine, stock, dairy, aromatics, vegetables) must carry the measurement.\n        • Partials of a larger ingredient — say it explicitly: 'half of the yellow onion — about ½ cup diced'.\n\n      The byte-identical match is the contract. If you write 'a glug of olive oil' or 'the wine' or '6 oz of butter' (when the ingredient row says '12 tbsp butter'), the runtime can't substitute and the cook reads numbers that disagree with their ingredient chips. ALWAYS cite the amount string from the ingredients[] row verbatim on first mention.>",
      "icon":        "🔪",
